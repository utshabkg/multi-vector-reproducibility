...
def analyze_centroid_coverage(
    ...
    if model_name == "constbert":
        embeddings_path = Path("indices/trec-tot-2025/constbert_tot_faiss_index/embeddings.npy")
        if not embeddings_path.exists():
            # Fall back to MS-MARCO
            embeddings_path = Path("indices/msmarco-passage/constbert_msmarco_faiss_index/embeddings.npy")
    else:
