...
def run_experiment(dataset_name: str, nlist: int, nprobe: int = 128):
    ...
    # Load dataset
    data_dir = Path(__file__).resolve().parent.parent.parent / 'data' / 'beir'
...
def main():
    ...
    for nlist in [1024, 4096, 8192]:
        ...
        output_path = Path(__file__).resolve().parent.parent.parent / 'results' / f'15_fever_nlist{nlist}.json'
    ...
    # Save full summary
    summary_path = Path(__file__).resolve().parent.parent.parent / 'results' / '15_nlist_hypothesis_summary.json'
