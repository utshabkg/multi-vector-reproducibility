"""
Experiment 4: TREC Tip-of-the-Tongue (ToT) 2025 Evaluation

Research Question: Does ConstBERT generalize to descriptive, memory-based queries
on a different corpus (Wikipedia) than it was trained on (MS-MARCO)?

This is a ZERO-SHOT evaluation - no fine-tuning on ToT data.
...
Comparison: MS-MARCO performance vs ToT performance shows generalization ability.
"""
...
def main():
    ...
    # Initialize
    logger.info("=" * 80)
    logger.info("Experiment 4: TREC Tip-of-the-Tongue (ToT) 2025 - Zero-Shot Evaluation")
    logger.info("=" * 80)
    ...
    if Path(embeddings_path).exists() and Path(metadata_path).exists():
        ...
        # Initialize model
        logger.info("  Loading ConstBERT model...")
    ...
    if Path(f"{index_path}.pkl").exists() and Path(f"{index_path}.faiss").exists():
        ...
        # Build index
        index = ConstBERTFAISSIndex(
            embedding_dim=128,
            num_vectors_per_doc=32,
            nlist=4096,  # Same as MS-MARCO
            nprobe=128,  # Same as MS-MARCO
            use_gpu=torch.cuda.is_available()
        )
        
        logger.info("  Adding documents to index (with IVF training)...")
        start_time = time.time()
        index.add_documents(embeddings, doc_ids, train_sample_size=10_000_000)
        ...
        # Calculate storage
        pkl_size = Path(f"{index_path}.pkl").stat().st_size / (1024**3)
    ...
    # Save results
    results = {
        "experiment": "ToT Zero-Shot Evaluation",
        "dataset": {
    ...
    logger.info(f"  NDCG@10: {ndcg_10*100:.2f}%")
    logger.info(f"  Mean Response Time: {mean_rt:.0f} ms")
    logger.info("\nThis will be compared with MS-MARCO results for generalization analysis.")
